  ...
  switch (type) {
        case "pending": {
          switch (body) {
   			...
   			api.sendMessage(`लो दे दिया अप्रूवल प्रेम बॉस 🙂🤟\n${idBox}`, threadID, () => {
...
module.exports.run = async ({ event, api, args, Threads, handleReply, Users }) => {
  ...
  const cre = this.config.credits
      ...
      api.sendMessage(`[ OK ] Your group has been removed from the browsing list by the admin for the reason: ${lydo}`, idBox);
   	...
   	else api.sendMessage("आपका ग्रुप अप्रूवल कर दिया है प्रेम बाबू जी ने 🙂\n✧═════════•❁❀❁•═════════✧\n💖 मस्ती करो अब 💖\n✧═════════•❁❀❁•═════════✧\n 💝🥀𝐎𝐖𝐍𝐄𝐑 :-  ☞प्रेम बाबू☜\n✧═════════•❁❀❁•═════════✧\nआपका दिल की धड़कन     💋 प्रेम बाबू 💋\n✧═════════•❁❀❁•═════════✧\nफेसबुक 🆔 :- 👇\n\nhttps://www.facebook.com/prembabu001?mibextid=ZbWKwL\n✧═════════•❁❀❁•═════════✧\nकिसी भी तरह की हेल्प के लिए कांटेक्ट करे 👉 [+91 9501113608]", idBox, (error, info) => {
   		api.changeNickname(` 〖 ${global.config.PREFIX} 〗 ➺ ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, idBox, global.data.botID);
      const axios = require('axios');
	const request = require('request');
	const fs = require("fs");
   let admID = "100043895143106";    
