...
(async function () {
	...
	logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "𝚃𝙷𝙸𝚂 𝙱𝙾𝚃 𝚆𝙰𝚂 𝙼𝙰𝙳𝙴 𝙱𝚈 𝙿𝚁𝙴𝙼 𝙱𝙰𝙱𝚄" : global.config.BOTNAME}`, "[ BOT INFO ]");
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        ...
        return api.sendMessage(`आपका रिक्वेस्ट प्रेम बाबू जी को भेज दिया है अब आपको अप्रूवल मिल जायेगा 🙂🤟`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`🙂 आपका ग्रुप में अप्रूवल नही मेरी जान\n 😗 तो अप्रूवल के लिए रिक्वेस्ट दो ऐसे 👉 ${prefix}request\n\n 💝 𝐎𝐖𝐍𝐄𝐑:- ☞𝐏𝐑𝐄𝐌 𝐁𝐀𝐁𝐔☜ 💫\n
😗 सबके दिल की धड़कन 👉 💋प्रेम बाबू💋\n

🙂 प्रेम बॉस की फेसबुक 🆔 लिंक 🙂 ☞ https://www.facebook.com/prembabu001?mibextid=ZbWKwL\n

🙂 अगर आपको अप्रूवल नही मिल रहा है तो मेरे बॉस प्रेम बाबू को डायरेक्ट एड कर सकते हो। 🙂🤟\n

मेरी जान पहले ग्रुप में रिक्वेस्ट लिख के सेंड करो ऐसे 👉 #request 🙂🤟`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
